...
export default function CheckoutPage() {
  ...
  async function handleSubmit() {
    ...
    if (method == 'card') {
      toast.promise(
        async () => {
          const { success, payload, error } = await CardCheckout(
            cartId,
            addresses.find((address) => address._id === value)!,
          );
      ...
      toast.promise(
        async () => {
          const { success, payload, error } = await CashCheckout(
            cartId,
            addresses.find((address) => address._id === value)!,
          );
          if (success && payload.status === 'success') {
          ...
          <div className="addresses">
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  {value
                    ? addresses.find((address) => address._id === value)?.name
                    : 'Select an address...'}
